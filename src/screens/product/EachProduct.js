...
import { Header, Left, Body, Container, Content, Item, Icon, Input, Button, Picker, Form } from 'native-base'
...
const EachProduct = ({ getProducts, product: { products, search, limit, sort, sortBy, overlay }, navigation }) => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        const getDataProducts = async () => {
            try {
                const response = await axios.get(`http://34.202.135.29:4000/api/v1/products?limit=${limitMask}&sort=${sortMask}&sortBy=${sortByMask}&search=${debouncedValue}&category_id=${category_id}`)
                getProducts(response)
            } catch(error) {
                console.error(error.message)
            }
    ...
    },[category_id, debouncedValue, limitMask, sortByMask, sortMask, overlayMask])
                        ...
                        <Content>
                            ...
                            <Header searchBar rounded style={styles.header}>
                               <Item>
                                 ...
                                 <Input placeholder="Search" onChangeText={value => onSearch(value)} />
                                 <Icon
                                   name="ios-funnel"
                                   onPress={ (e) => handleOpenOverlay(e) }
                                 />
                               </Item>
                               <Button transparent>
                                 <Text>Search</Text>
